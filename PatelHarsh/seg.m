...
I_INDEX=0;
J_INDEX=0;
% Uncomment this region for obtaining the matrix W
%  for ii = 1:size(base_col,2)
%      for jj = 1:size(base_col,1)
%          for kk = 1:size(base_col,1)
...
%                     % [i j row col]
%                     %if(memo(I_INDEX,J_INDEX) == 0)
%                      aff = double(exp( -1 * (sqrt((base_pad(i,j) - base_pad(row,col))^2 )) / sigma_I^2 ) * exp(-1 * (sqrt((col-j)^2 + (row-i)^2)) / sigma_L^2 ));
%                      %aff
%                      
